...
const ToolsSentimentFlag = () => {
  const { t } = useTranslation();
  const { activeItem, setActiveItem } = useToolsContext();

  if (activeItem !== 'sentiment') return null;
  ...
  return (
    <>
      <MenuButton onClick={() => setActiveItem('menu')}>
        ...
        <Span isBold>{t('__TOOLS_SENTIMENT_PREVIOUS_ITEM')}</Span>
      </MenuButton>
      <Separator />
      <Body>
        <MD>{t('__TOOLS_SENTIMENT_CONTENT')}</MD>
      </Body>
    </>
  );
};

export { ToolsSentimentFlag };
