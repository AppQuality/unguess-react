...
export const SortBy = () => {
	...
	const makeTitle = (fieldName: string): string => {
		...
		return `${t('__BUGS_ORDER_BY')} ${t_fieldName.charAt(0).toUpperCase() + t_fieldName.slice(1)}`
	};
	...
	return (
		<Dropdown
			...
			onSelect={(selectedKey: string) => {
				availableSelections.map(availableSelection => availableSelection.key === selectedKey &&
					(() => {
