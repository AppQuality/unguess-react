...
export const AgeRange = (props: FormikProps<WizardModel>) => {
  ...
  if (ageRange === 'all')
    return (
      <Trans i18nKey="__EXPRESS_4_WIZARD_STEP_RECAP_WHO_AGE_RANGE_TEXT_ALL">
  ...
  return range.length ? (
    <Trans
      i18nKey="__EXPRESS_4_WIZARD_STEP_RECAP_WHO_AGE_RANGE_TEXT"
      components={{
        span: <Span isBold />,
      }}
      values={{
      ...
      default="Are aged between <span>{{ age_range_start }}</span> and <span>{{ age_range_end }}</span> years."
    />
  ) : null;
};
